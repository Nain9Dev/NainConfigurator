...
The initial MVP publishes one modular desk (`DESK-001`) in company locale `es-ES`. The platform must nevertheless support future companies and fundamentally different products through data, without desk-specific database columns, request properties or validation branches.
...
`00.2-CommercialStrategy.md` is referenced by several documents but is **not published in this repository**. Target market, revenue structure, packaging and cost-to-revenue ratios are private business decisions maintained outside version control. Every cross-reference to it in this documentation set is a pointer to that private record, not a missing file.
...
## Operating model relevant to the codebase

- One shared, strictly company-scoped multi-company service; no per-customer code, schema, repository or release fork.
- Catalog publication is an operational process, not a customer-facing self-service feature during the MVP.
- Branding is constrained, data-driven co-branding; it never changes commercial state.
...
- Infrastructure cost is measured per environment and per company against an approved budget before any capacity upgrade.
